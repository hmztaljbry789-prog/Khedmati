...
export default function AdminUsers() {
    ...
    const approveVerify = async (u) => {
        ...
        try {
            ...
            await load();
            toast(isRtl ? "تم قبول توثيق الفني وإرسال إشعار له" : "Provider approved and notified", "success");
        } catch (e) {
            console.error(e);
            toast(e?.message || t.reqNotMet, "error");
        } finally {
            setBusyId(null);
        }
    };
    ...
    const rejectVerify = async (u) => {
        const reason = window.prompt(
            isRtl ? "اكتب سبب الرفض بوضوح (5 أحرف على الأقل):" : "Enter a clear rejection reason (at least 5 characters):",
            u.verificationRejectionReason || ""
        );
        if (reason === null) return;
        if (reason.trim().length < 5) {
            toast(isRtl ? "سبب الرفض قصير جدًا" : "Rejection reason is too short", "error");
            return;
        }
        setBusyId(u._id);
        try {
            ...
            await load();
            toast(isRtl ? "تم رفض الطلب وإرسال السبب للفني" : "Request rejected and reason sent", "success");
        } catch (e) {
            toast(e?.message || (isRtl ? "تعذّر رفض الطلب" : "Could not reject request"), "error");
        } finally {
            setBusyId(null);
        }
    };
    ...
    const remove = async (id) => {
        const confirmed = await confirm(t.deleteConfirm, {
            danger: true,
            confirmLabel: isRtl ? "نعم، احذف" : "Yes, delete",
            cancelLabel: isRtl ? "تراجع" : "Cancel",
        });
        if (!confirmed) return;
        setBusyId(id);
        try {
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex flex-wrap gap-1.5">
                                                    ...
                                                    {u.isVerified === false && (
                                                        ...
                                                        >
                                                            {isRtl ? "رفض مع السبب" : "Reject with reason"}
                                                        </button>
                                                    )}
                                                </div>
